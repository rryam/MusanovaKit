...
struct LyricsParserTimedTests {
  ...
  @Test
  func testParserProducesTimedSegments() throws {
    let data = try loadFixture(named: "lyricsTimedCallOutMyName", withExtension: "json")
    ...
    #expect(!firstLine.segments.isEmpty)
    #expect(firstSegment.text == "We")
    #expect(abs(firstSegment.startTime - 9.305) < 0.0001)
    #expect(abs(firstSegment.endTime - 9.723) < 0.0001)
  }

  @Test
  func testParserHandlesPlainParagraphWithoutSpans() throws {
    let ttml = makeTTML(divContent: "<p>Just some plain text line</p>")
    let parser = LyricsParser()
    let paragraphs = parser.parse(ttml)
  ...
  @Test
  func testParserParsesMinuteTimecodes() throws {
    let ttml = makeTTML(divContent: "<p><span begin=\"1:02.915\" end=\"1:03.100\">Late</span></p>")
    let parser = LyricsParser()
    let paragraphs = parser.parse(ttml)
  ...
  @Test
  func testParserDefaultsTimingWhenBeginMissing() throws {
    let ttml = makeTTML(divContent: "<p><span end=\"2.5\">Hi</span></p>")
    let parser = LyricsParser()
    let paragraphs = parser.parse(ttml)
  ...
  @Test
  func testParserHandlesMultipleSongPartsFromTimelessFixture() throws {
    let data = try loadFixture(named: "lyricsTimedTimeless", withExtension: "json")
    let response = try JSONDecoder().decode(MusicLyricsResponse.self, from: data)
    ...
    #expect(verseLine.segments.count >= 5)
    #expect(verseLine.segments.first?.text == "Ever")
    if let segment = verseLine.segments.first {
      #expect(abs(segment.startTime - 33.906) < 0.0001)
    }
  }
  ...
  private func makeTTML(divContent: String) -> String {
    """
    <tt xmlns=\"http://www.w3.org/ns/ttml\" xmlns:itunes=\"http://music.apple.com/lyric-ttml-internal\" xmlns:ttm=\"http://www.w3.org/ns/ttml#metadata\">
      <body>
        <div>
